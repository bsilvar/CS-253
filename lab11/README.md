...
```
valgrind --tool=memcheck --leak-check=yes --show-reachable=yes bin/myls -s -d /usr/bin 
...
==154461== Command: bin/myls -s -d /usr/bin 
...
1. Expand the folder named src and open warmup.c
...
### Problem Description
This is a continuation of the File System Scanner from the LabWarmup.  Begin by copying warmup.c from the LabWarmup into src/activity.c. Also, update Makefile to build activity.c rather than warmup.c. 
...
### Implementation Guide
1. Expand the folder named src and copy warmup.c
2. Enter the program code to create an application as described in the Problem Description.
